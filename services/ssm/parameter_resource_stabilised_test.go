...
type SSMParameterResourceStabilisedSuite struct {
	suite.Suite
}

func (s *SSMParameterResourceStabilisedSuite) Test_stabilised() {
	...
	testCases := []plugintestutils.ResourceHasStabilisedTestCase[*aws.Config, ssmservice.Service]{
		{
			Name:           "a parameter is always stabilised",
...
func TestSSMParameterResourceStabilisedSuite(t *testing.T) {
	suite.Run(t, new(SSMParameterResourceStabilisedSuite))
}
