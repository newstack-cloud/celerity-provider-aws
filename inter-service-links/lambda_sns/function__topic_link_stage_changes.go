...
func (l *functionTopicLinkActions) StageChanges(
	ctx context.Context,
	input *provider.LinkStageChangesInput,
) (*provider.LinkStageChangesOutput, error) {
	changes := &provider.LinkChanges{}

	annotations := getTopicLinkAnnotations(
	...
	if pluginutils.IsResourceNew(input.ResourceAChanges) ||
		...
		// The execution-role permission data for this link references the topic ARN and
		// is only known at deploy time.
