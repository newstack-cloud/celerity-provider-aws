...
func (l *functionEventBusLinkActions) StageChanges(
	ctx context.Context,
	input *provider.LinkStageChangesInput,
) (*provider.LinkStageChangesOutput, error) {
	changes := &provider.LinkChanges{}

	annotations := getEventBusLinkAnnotations(
	...
	envVarName := eventBusEnvVarName(annotations.envVarName, &input.ResourceBChanges.AppliedResourceInfo)
	...
	if pluginutils.IsResourceNew(input.ResourceAChanges) ||
		...
		// When either resource is (re)created, the execution-role permission data for
		// this link changes but is only known at deploy time (it references the bus ARN).
